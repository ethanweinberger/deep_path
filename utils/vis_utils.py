...
def get_slide_patch_confidences(test_slide):
    ...
    patch_confidences_values = filter(lambda x: get_patch_confidence(x), full_test_slide_patches) 
    return map(lambda x: PatchAndConfidence(x[0], x[1]), zip(full_test_slide_patches, patch_confidence_values))
...
def classify_whole_slides(
    ...
    with tf.Session(graph = graph) as sess:
        ...
        pos_slide_patch_confidences = map(
            lambda x: get_slide_patch_confidences(x), pos_slide_name_list)  
        neg_slide_patch_confidences = map(
            lambda x: get_slide_patch_confidences(x), neg_slide_name_list)

        all_patch_confidences = list(itertools.chain(pos_slide_patch_confidences)) + list(itertools.chain(neg_slide_patch_confidences))
